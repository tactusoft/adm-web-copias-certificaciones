...
public class PDFGeneratorService {
    ...
    protected final static Font fontBlackSmall = new Font(Font.FontFamily.HELVETICA, Constantes.PDF_FOOTER_FONT_SIZE, Font.NORMAL);
    ...
    public static ByteArrayOutputStream createPdf(String content, String title, String subject, String keywords, boolean isCertificacion, String barcode)
        ...
        try (ByteArrayOutputStream bos = new ByteArrayOutputStream()) {
            ...
            try (ByteArrayInputStream input = new ByteArrayInputStream(content.getBytes(Constantes.UTF_8))) {
                ...
                worker.parseXHtml(writer, document, input, null, Charset.forName(Constantes.UTF_8), new XMLWorkerFontProvider(pathFonts), pathResources);
            }
    ...
    public static void AddSignPage(String source, String target, Cesl_detalleSolicitud detalle, Persona radicador)
        ...
        try (ByteArrayOutputStream bos = new ByteArrayOutputStream()) {
            ...
            mainTable.setTotalWidth(PageSize.LETTER.getWidth() - PDFHeaderFooter.MARGIN_LEFT
                    - PDFHeaderFooter.MARGIN_RIGHT);
            mainTable.setLockedWidth(true);

            Paragraph p1 = new Paragraph(
                    String.format("La Secretaría General AD HOC de la Superintendencia de Industria y Comercio certifica que la firma estampada en este documento corresponde a %s, y se encuentra registrada en ésta Superintendencia.\n\nSe expide en Bogotá a los %s día(s) del mes de %s de %s.", nombreSecretarioCamra, dia, descMes, ano), fontBlack);
            ...
            PdfPCell cellGrafo = event.getResourceImageAsPDFCell(String.format("FIRMA_SECRETARIO_AD_HOC_%s.PNG", Constantes.WS_CANCILLERIA_ID_AUTORIDAD_SECRETARIO_AD_HOC));
